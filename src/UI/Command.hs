...
module UI.Command (
  parseAndRunCommand -- ^ St -> B.EventM MainWindowName (B.Next St)
  , runCommand -- ^ Command -> St
               -- -> Either String (B.EventM MainWindowName (B.Next St))
  ) where
...
runCommand ::
  Command -> St -> Either String (B.EventM MainWindowName (B.Next St))
...
runCommand (CommandInsert e) st =
  ...
  where
    f :: (Rslt, Addr) -> B.EventM MainWindowName (B.Next St)
